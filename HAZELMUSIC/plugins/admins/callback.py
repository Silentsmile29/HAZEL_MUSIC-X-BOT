...
from HAZELMUSIC import YouTube, app
from HAZELMUSIC.core.call import HAZEL
from HAZELMUSIC.misc import SUDOERS, db
from HAZELMUSIC.utils.database import (
    get_active_chats,
    get_lang,
    get_upvote_count,
    is_active_chat,
...
from HAZELMUSIC.utils.decorators.language import languageCB
from HAZELMUSIC.utils.formatters import seconds_to_min
from HAZELMUSIC.utils.inline import close_markup, stream_markup, stream_markup_timer
from HAZELMUSIC.utils.inline.play import panel_markup_1, panel_markup_2, panel_markup_3
from HAZELMUSIC.utils.stream.autoclear import auto_clean
from HAZELMUSIC.utils.thumbnails import get_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await HAZEL.pause_stream(chat_id)
        buttons = [
        [
        ...
        await HAZEL.resume_stream(chat_id)
        buttons_resume = [
        [
        ...
        await CallbackQuery.answer()
        await HAZEL.stop_stream(chat_id)
        ...
        await HAZEL.mute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await HAZEL.unmute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        if command == "Skip":
            ...
            try:
                ...
                if not check:
                    ...
                    try:
                        return await HAZEL.stop_stream(chat_id)
                    except:
                        return
            except:
                try:
        ...
        if "live_" in queued:
            ...
            try:
                ...
                image = None
            try:
                await HAZEL.skip_stream(chat_id, link, video=status, image=image)
            except:
            ...
            try:
                ...
                image = None
            try:
                await HAZEL.skip_stream(chat_id, file_path, video=status, image=image)
            except:
                return await mystic.edit_text(_["call_6"])
            button = stream_markup(_, chat_id)
            img = await get_thumb(videoid)
            ...
            try:
                await HAZEL.skip_stream(chat_id, videoid, video=status)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
            button = stream_markup(_, chat_id)
            run = await CallbackQuery.message.reply_photo(
            ...
            if videoid == "telegram":
                ...
                try:
                    ...
                    image = None
            try:
                await HAZEL.skip_stream(chat_id, queued, video=status, image=image)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
            if videoid == "telegram":
                button = stream_markup(_, chat_id)
